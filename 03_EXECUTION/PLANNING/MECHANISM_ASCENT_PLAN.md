...
5. **Choose the next macro transfer from the proven exit.** The owner's request for faster progress through reusable macro primitives is developed in [the next ascent proposal](NEXT_ASCENT_PROPOSAL.md): compare route emphases, target a connected +66 m slice, and close the first +11 → +33 m transfer before implementing it. These are planning targets, not demonstrated routes. A contained heavy roller and pendulum or falling stair slabs remain options; the skyline, old band cuts or eleven-phase inspiration do not imply completed connections.
...
Spend geometric and simulation detail where it changes a visible action. Favour a few large working bodies and broad receiving surfaces. Small bearings/retainers can exist, but they cannot carry the entire gameplay through invisible sequencing. No compulsory pin/cable/tool delivery chores ahead of every simple gravitational event. A control releases or redirects a known finite source; it does not conjure its work.

...
**Status:** SCREW GREEN-VERIFIED at c463439 / run 36160560044. WATER-WEIGHT LIFT GREEN-VERIFIED at 2849550 / run 36205330060. Dock-to-+12 m stair connection implemented and locally native-verified; Fold/Android CI proof pending for this connection.
...
**Dock-to-stair receiving seam:** the fixed dock ends at `z=-109.85 m`; `MOD-STAIR-A`'s +8 m landing begins at `z=-114.2 m`. Walking toward that landing without a jump fell to the lower stair, while a timed jump did clear the opening. The added grating spans `z=-109.60..-114.70 m`, overlaps both load-bearing ends, and carries a visible/collidable 0.70 m transverse brace joining its side girders. The existing player jump clears it, air braking settles back onto the grating, and walking onto the existing stair climbs to its stable +12 m landing. The gate still blocks grade entry to the stair, while this lift creates an alternative +8 m entrance. The native integration run starts with filling/boarding/riding the cage, then uses real controls across this seam; full Fold/Android proof remains pending.

...
| **Next code job** | **`AS-004`** — seat needles at 96 m, carrying `CAP-HOOK5`. Revalidated against `AS-003` as built: **blocked on a design decision**. The tower's own stair (`build_stack()`) is walkable from grade to 154 m; its 44 m deck drops onto the +40 m hall deck and the hall deck jumps into its 33 m deck; its balconies reach the planned 48 m needle rack and 96 m seat. Close that stair below and above the hall deck with a real access constraint, or keep it as a route and make `AS-004` optional (`AS-004_NEEDLE_SEAT.md`, Revalidation) |
...
> grade to 154 m and connects both ways with the +40 m hall deck and this slice's members.
> Close it with a real access constraint (Law 17 allows that, not an invisible blocker),
> or keep it and make `AS-004` optional. Its Revalidation section has the measured reaches.
>

...
| **Next code job** | Restore the current full proof path to GREEN, then build **only the ground Archimedes screw water lift** from `03_EXECUTION/PLANNING/GROUND_WATER_ASCENT.md`. Its receiving bucket/cage lift is fully specified there as **PLANNED / NOT BUILT** and is a later mechanism cycle. `AS-006` Stages B and C are paused, not complete. |

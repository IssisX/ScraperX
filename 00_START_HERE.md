...
| **Next code job** | **`AS-004`** — seat needles at 96 m, carrying `CAP-HOOK5`. Gate satisfied (`AS-003` implemented). Revalidate it against `AS-003` as built before coding: the cage moved 18 m north, the carry is at belly height, and the block reaches the hall deck by `MOD-STAIR-A` |
...
| `AS-004` | seat needles at 96 m | B01 | PLANNED | re-derived here | none — **next code job** |
...
> implementation gate satisfied (`AS-003` implemented). Revalidate it against `AS-003` as built
> first -- its Result record lists what moved.
>

...
| Evidence | Local Godot 4.7 validation passed: default body removal/input/checkpoint, ordinary stairs through all 14 flights (player centre Y=154.9), full native suite, 23 legacy UI scenarios, default rendered input proof and both collision exports. Current candidate still requires the exact-source CI Android compile/export and artifact gates. Existing green restoration runs prove older source only; no completed CI result is asserted in this pre-publication record. |

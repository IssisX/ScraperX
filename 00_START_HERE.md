...
| **Design doctrine** | `03_EXECUTION/PLANNING/CAUSAL_ASCENT_DOCTRINE.md`. A lift is a real energy chain: stored energy, a restraint, a physical trigger, a load path, governed motion, a place for the leftover energy, a real catch, then a physical handoff. Stages in that file from 280 m to 680 m are **not built**. Until the ground water-weight cage lift is in source and its proof is green, that cage is the only new machine allowed. It is not in source at `14363a3`. |

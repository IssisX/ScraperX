...
| **Design doctrine** | `03_EXECUTION/PLANNING/CAUSAL_ASCENT_DOCTRINE.md`. Timeless physics rule for a lift: stored energy, a restraint, a physical trigger, a load path, governed motion, a place for leftover energy, a real catch, then a physical handoff. The 280 m to 680 m stages in it are a design. That file does not hold branch state, run numbers, or a work queue. |
...
| `AS-010` | service cage off TP-640, 22 m | B06 | IN PROGRESS | re-derived here: one counter-mass lift, not the band | `PASS scraperx_sim AS-010 M` (local): no hook, cage stays; hooked, rider to 663.3 m, skip drop pays for cage and rider |

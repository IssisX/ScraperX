...
| `AS-003` | `CAP-HOOK5` acquire | B00 | **IMPLEMENTED** | re-derived here | falsifiers green locally (`AS-003 apron`, `cage`, `Hook5 Rack`; `touch_carry`) |
| `AS-004` | seat needles at 96 m | B01 | PLANNED | re-derived here; revalidated, blocked on the tower-stair decision | none — **next code job** |
...
> **Next code job: `AS-004` — seat needles at 96 m.**
> `03_EXECUTION/ASCENT/AS-004_NEEDLE_SEAT.md`. Lifecycle `PLANNED`, provenance re-derived here,
> implementation gate satisfied (`AS-003` implemented) and revalidated against it as built.
> **Blocked on one design decision** before any code: the tower's own stair is free from
> grade to 154 m, its 11-44 m decks land on the `AS-001`/`AS-002` bodies, and its balconies
> reach this slice's members. Close its four flights below 44 m with a real access
> constraint (Law 17 allows that, not an invisible blocker), or keep it and make `AS-004`
> optional. Its Revalidation section has the measured reaches and the cut audit.
>

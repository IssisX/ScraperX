...
| **Next code job** | The apron chain in `03_EXECUTION/PLANNING/COLOSSUS_APRON.md`. Ground level. Pipes, scale, trapdoor, wrecking ball, slabs, seesaw. The player rides the seesaw up about 12 m. The old mechanism-ascent plan is withdrawn. |
| **Next authoring job** | re-author **`AS-007`** (B03, 220 → 340 m) as the next linked-stage band, once `AS-006` is built |
| **Authoring contract** | `03_EXECUTION/PLANNING/COLOSSUS_APRON.md` is the build. `MECHANISM_ASCENT_PLAN.md` is withdrawn. |
| **Design doctrine** | The laws in `03_EXECUTION/PLANNING/CAUSAL_ASCENT_DOCTRINE.md` still stand (a fall pays for a rise, a real body under the player). The thirteen stages in that file are withdrawn. |

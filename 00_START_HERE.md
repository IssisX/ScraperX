...
| **Next code job** | **Mechanism ascent, Step 1**, once `AS-006` is re-authored — the rigging kit and the first lift above the stair: a counter-mass lift off the 154 m deck that the player completes with a hook and a release pin. Plan: `03_EXECUTION/PLANNING/MECHANISM_ASCENT_PLAN.md` §5 |
| **Next authoring job** | re-author **`AS-006`** (B02) against this branch as the first linked-stage band, 154 → 220 m, per the mechanism ascent plan §6. It comes first: Step 1 codes its first stage |
...
| `AS-006` | counterweight ride / pin dump | B02 | PLANNED | ⚠ imported — **next authoring job**: re-author as the first linked-stage band, 154 → 220 m | none |
...
> **Next job, in order: re-author `AS-006`, then code Mechanism ascent Step 1.**
...
> completes and rides. Step 1 is the rigging kit (hook and unhook cable ends, pins, chocks,
> wedges, drag and shove) and the first stage: a counter-mass lift off the 154 m deck. Its
> falsifiers: no link, no lift; the payload's energy gain never exceeds what the falling mass
> released; the rider inherits the cage's motion.
>
> `AS-006` (B02) is re-authored first, as the first linked-stage band, 154 → 220 m, per the plan's
> §6; Step 1 then codes its first stage. `AS-004` stays `PLANNED`: the stair stays (owner's
> direction), so its needles are no longer the way to 96 m.

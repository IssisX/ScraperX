...
Plan axes: +X east, +Y north. These are **Atlas tower-local** coordinates.
The current native/Godot world is Y-up, with the tower centre at source
`(0, 0, -150)` and the apron walking datum y=0. For comparing a tower-local
Atlas point to this placement: `source = (atlas.x, atlas.z, atlas.y - 150)`;
inverse: `atlas = (source.x, source.z + 150, source.y)`. Existing source constants
and generated world solids are already world coordinates: never translate them
again. A coordinate table must state its frame and whether its height is a body
centre, centre of mass or walking surface. Historical source comments using
opposite facade names do not change these coordinates.
...
The envelope/grid above are campaign design targets, not measurements of the
current source footprint. At `7e66eb6`, the existing tower base is 52 m wide
(half-width 26), with its own generated frame, 11 m stair levels to 154 m and
22 m ring levels above. Reconcile new modules against that actual geometry;
do not scale or relocate proven routes to force the target grid to fit.
...
**Opening.** The player reaches the existing intake from the apron. In current
source, the 4 t pack starts on the apron at x=0, z=-112.2, physically obstructing
`MOD-DOG-A`; it is outside the moving belt's x=4..8 span. The yard crane's controls
are reachable by a grade ramp; no cold-pendant acquisition gate is implemented.
The water screw, conserved tank and water-weight cage provide another route to
+8 m, followed by a supported connection into the stair. Acquisition of the separate
hook block belongs to AS-003, not to crane activation. The complete tower stair
to 154 m remains a legal route (owner decision, 2026-09-24).
...
| `MOD-HOOK5-RACK` | 36 kg hook block in a +4.45 m roof cage reached by a belt-assisted jump/hang or other legal machine route; internal 38 kg bar obstructs its inward door | capability |
...
| `MOD-STAIR-A` | Six fixed flights to +24.1872 m, with gate controlling the grade throat; water route enters at +8 m; AS-002 supplies the +24→40 continuation | traversal |
| `MOD-STAIR-A-SWING` | 400 kg dynamic swinging flight from +24 to +32 m under actual rope tension; shared fixed upper flight continues to +40.1872 m | freight + traversal |
| `MOD-CW-CRADLE` | 500 kg guided cradle; seated 4 t pack supplies gravity work/tension for the swinging flight | freight |
| `MOD-SKIN-LADDER-S` | Existing stepped ledges to +24, five more to +32, walkway and shared upper flight to +40; uses current mantle geometry, not a ladder verb | SKIN |

**Example couplings**

1. Use the finite yard crane to lift the 4 t pack clear of the gate sweep. The driven dog opens the grade throat; the fixed stair reaches +24 m. A manual 200–400 kg shove capability is not evidence that the player can move this 4 t load.
2. Belt still runs if not isolated; riding the belt is valid moving-support traversal.
3. Ignore the crane. Climb the real ledges to +32 m, cross the walkway and ascend the shared upper flight to the hall at +40.1872 m. Freight stays unsolved. A hypothetical chain dropped from above is not an implemented attachment interface.
...
5. From the caught cage, step onto the +8 m dock. A short supported grating crosses the intake wall to `MOD-STAIR-A`'s +8 m landing; jump its solid transverse brace and climb the existing flight to the +12 m landing. This enters the stair above `MOD-DOG-A` and therefore bypasses that gate for the first ascent. It does not deploy the optional +24–32 m swinging flight; outside structure and the retained tower stair remain legal. The original dock-to-stair jump remains possible.

**Fall geography.** Apron is the primary chute landing. Survival depends on actual drop, impact and landing; a 24–40 m fall is not presumed survivable. Falling *onto* the belt while it moves is a moving-support problem, not a kill plane.

**Exit states that make B01 reachable**

Stand on the +40.1872 m hall after either the freight-driven swinging flight and
upper stair, or the outside ledges/walkway and shared upper stair. The independent
tower stair is also legal. Clearing the grade gate alone reaches +24 m, not +40.
The 11.5 m-high yard crane cannot place the player on a +40 m soffit.

Checkpoint intent remains stable support at z≥40 m. This is not a statement that
all upstream intake bodies are currently captured; AS-001/002 record that gap.

---

### B01 — Transfer Hall (40–120 m)

**Physical job.** Optional structural seating/freight capability beside the
retained tower stair. AS-004/005 are unbuilt and BLOCKED on supported approaches,
rigging alignment, guide/load law and landing/recovery closure. Missing needles
must affect that machine through a real load path, not prevent every ascent.
...
1. A supported finite hoist associated with `MOD-CAGE-1` may place needles at 96 m once access, rigging and load path are resolved. The grade yard crane cannot reach this operation. Actual seating restores guide alignment only through the declared structural law.
2. One needle may become a route to exterior structure only after its real bearing/retention, load capacity and receiving geometry are derived. Elastic motion requires an elastic model; a seating count cannot activate a floor.
3. Leave needles. The existing tower stair is a legal route through 120 to 154 m. The optional east-outrigger connection still requires geometry/controller proof; it is not a built ladder route.
...
**Physical job.** Linked counterweight machinery above the existing 154 m stair,
with meaningful leftovers and recovery. AS-006 owns the source-derived mechanism
contracts; the 120–154 m part of this band is already served by the retained stair.

| ID | Current interpretation / boundary |
|---|---|
| `MOD-CW-STACK` | Counter-mass machinery family: implemented A skip and B guided lattice with their cages, frames, ropes and catches; local native identities in AS-006 |
| `MOD-DRUM-LOW` | Reserved optional drum/rearm equipment; no 128 m drum-room implementation is assumed |
| `MOD-CW-PIN` | Physical release/retention role; implemented catch levers/trip lines are described in AS-006, not a magic stack-mass toggle |
| `MOD-WELL-LEDGES` | Required support/recovery connections; existing rings at 176.25/198.25/220.25 m do not prove continuous access or 8–10 m ledges |

**Current chain.** Reach A on the 154 m deck, transfer its slack rope end from
bollard to cage, pull the physical trip handle; 800 kg skip falls 22 m and cage
rises to 176.25 m. Transfer into B; rig and release the 2500 kg guided lattice,
which descends 22 m while B rises to 198.25 m. B's lattice then spans
176.25..198.25 m; it is not the old swinging derrick and does not reach 220 m.

**Remaining contract.** C and its proposed conserved-rubble reset of A, plus an
independent climb to 220, are unbuilt. AS-006 lists the fill-before-release,
outlet-height, unloading, energy and recovery blockers. No completion claim for
the whole band until both intended ascent and legal alternative are exercised.

**Fall geography.** Real well contacts/clearance and impact govern survival;
a parachute is neither guaranteed to open in the shaft nor a powered return.
Checkpoint recovery must restore the complete relevant mechanism state.

**Exit.** Stable player support on the +220.25 m ring, physically connected to
AS-007's service approach. Current implemented A/B output is only +198.25 m.

---

### B03 — Wet Isolation (220–340 m)

**Physical job.** Process changes real hazard and machinery access. Proposed
water service, drainage and a physical wet lock must earn the route; AS-007 is
BLOCKED on its entry, supported geometry and dimensional process law. The chosen
water design is not an implemented campaign fluid system.
...
| `MOD-ISO-STAIR` | Solid stair whose water hazard changes with actual flow/level; collision remains truthful in wet and dry states |
| `MOD-WET-LOCK` | Door/counterweight with a dimensioned physical load or latch mechanism; no predicate-frozen pose |

**Example couplings**

1. Reach dry service controls; isolate upstream, bleed to reduce local differential pressure, then safely fit/retain the blind and drain into a sized receiver. Reduced water hazard and physically released lock permit the stair route. AS-007 must close each step before implementation.
2. Dump the header into the sump on purpose to drown a fire below (if that fire exists later). Stair hazard increases while its solids remain; a separately proven exterior route must remain reachable.
3. A legal exterior bypass is required but unbuilt. Workers, if later active, respond to actual hazard/access state; no invisible support removal or navigation permission substitutes for it.
...
Representative content contracts; implemented and blocked portions are distinguished in §6 and the ascent ledger:

| Chain | ACT | STATE | WORLD | PLAY |
|---|---|---|---|---|
| K0 Intake | crane clears pack, then seats it on cradle | dog clears; cradle tensions rope | grade stair opens; swinging flight deploys | +24→32→40 route; outside/tower/water alternatives remain legal |
| K1 Needles | rig/seat needles | supported guide load path restored | real alignment/brake allows landing | optional ride to TP-120; BLOCKED |
| K2 Pin | rerig rope and pull catch lever | counter-mass released through real rope | guided cage rises; mass remains below | A/B to 198.25; C/recovery to 220 BLOCKED |
| K3 Blind | isolate, bleed, fit blind, drain | conserved flow/pressure changes | water hazard and physical lock load reduce | intended FLOW to TP-340; BLOCKED |
...
| K8 Fans | isolate drive and engage real lock | finite rotor energy dissipated; lock bears torque | stopped blade/bell provides real contact | intended mast interior climb |

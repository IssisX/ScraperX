...
int main() {
    ...
    for (auto id : ids) {
        ...
        if (b.mMax.GetY() > 60.0F) continue;
        if (b.mMax.GetX() < -40 || b.mMin.GetX() > -26.5F || b.mMax.GetZ() < -152 || b.mMin.GetZ() > -138) continue;
        const float wx = b.mMax.GetX() - b.mMin.GetX(), wz = b.mMax.GetZ() - b.mMin.GetZ();
        (void)wx; (void)wz;   // too narrow to stand on
        std::printf("ent=%3llu bot=%6.2f top=%6.2f x[%6.2f,%6.2f] z[%8.2f,%8.2f]\n", (unsigned long long)bi.GetUserData(id), b.mMin.GetY(), b.mMax.GetY(),
